...
Note there are some differences of this `flights` data from that in tidyverse. The data types for some variables are different. There are no natural ways in Pandas to hold integer column with missing values; so `dep_time` , `arr_time` are `float64` instead of `int64`. The `time_hour` column in tidyverse seems missing in Python.
```{python}
flights.info()
```
...
transform(flights, [:arr_delay, :dep_delay] => (-) => :gain)
insertcols!(flights, :gain => flights.arr_delay - flights.dep_delay)
```
...
```{julia}
combine(flights, "dep_delay" => x -> mean(skipmissing(x)))
```
