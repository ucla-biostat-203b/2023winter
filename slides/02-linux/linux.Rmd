...
- To practice the Linux commands, we can log on the teaching server, git clone the course material, and try the Linux commands on the Terminal.   
  ...
  1. Log onto the RStudio on teaching server: <http://server.ucla-biostat-203b.com:8787>  
  
  2. In RStudio, `File` -> `New Project...` -> `Version Control` -> `Git` -> put `https://github.com/ucla-biostat-203b/2022winter.git` to `Repository URL:` -> put `203b-2022winter` to `Project directory name:` -> choose home directory for `Create project as subdirectory of:` -> click `Create Project` button
  
  3. In the `Terminal` tab, navigate to the folder of current slide: `cd ~/203b-2022winter/slides/02-linux`
...
- For Windows users, there are at least three ways: (1) (highly recommended) [Git Bash](https://git-scm.com/download/win) which is included in Git for Windows, (2) (not recommended) [PuTTY](http://www.putty.org) program (free), or (3) (may be an overkill for this class)  use WSL for Windows to install a full fledged Linux system within Windows.
...
- _Private key_. Put on your own computer. Consider this as the actual key in your pocket; **never give private key to others**. For fun: <https://www.youtube.com/watch?v=S8K464ImU0c>
...
- Demo script [`meanEst.R`](https://raw.githubusercontent.com/ucla-biostat-203b/2022winter/main/slides/02-linux/meanEst.R) implements an (terrible) estimator of mean
$$
...
- [`runSim.R`](https://raw.githubusercontent.com/ucla-biostat-203b/2022winter/main/slides/02-linux/runSim.R) has components: (1) **command argument parser**, (2) method implementation, (3) data generator with unspecified parameter `n`, and (4) estimation based on generated data.
```{bash, echo=FALSE}
cat runSim.R
```
...
- Suppose we have 
    - [`runSim.R`](https://raw.githubusercontent.com/ucla-biostat-203b/2022winter/main/slides/02-linux/runSim.R) which runs a simulation based on command line argument `n`.  
...
- Let's demonstrate using the script [autoSim.R](https://raw.githubusercontent.com/ucla-biostat-203b/2022winter/main/slides/02-linux/autoSim.R)
    ```{bash}
    cat autoSim.R
    ```
