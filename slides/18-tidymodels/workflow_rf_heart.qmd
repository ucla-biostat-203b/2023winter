...
3. Tune the random forest using 10-fold cross-validation (CV) on the non-test data.
...
Heart_other_test <- testing(data_split)
dim(Heart_other_test)
```
...
- numerical features: Age, RestBP, Chol, MaxHR, Oldpeak 

- categorical features coded as integer: Sex (0 or 1), Fbs, RestECG, ExAng, Slope, Ca 

- categorical features coded as string: ChestPain, Thal

- There are missing values in `Ca` and `Thal`. Since missing proportion is not high, we will use simple mean (for numerical features) and mode (for categorical features) imputation. 
...
preproc_recipe <- 
  ...
  prep(training = Heart_other[, -1], retain = TRUE)
preproc_recipe
```
...
folds <- vfold_cv(Heart_other[, -1], v = 5)
folds
```
