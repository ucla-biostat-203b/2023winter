...
This tutorial is for Mac users. It goes through the process of installing the software environment (R, Quarto, RStudio, Git, SSH, Docker) for reproducing 203B course materials and working on 203B homework.

For Windows users, please first read the [tutorial](https://ucla-biostat-203b.github.io/2023winter/labs/lab01/lab01_windows.html) to install `Git for Windows`, which ships with `Git Bash`. Then most bash commands in this tutorial will work in `Git Bash`.
...
- In Terminal (Mac Terminal, Git Bash, or WSL), you can `git clone` a copy of course materials by
```{bash}
#| eval: false
...
- In Terminal (Mac Terminal, Git Bash, or WSL), you can run
```{bash}
#| eval: false
...
- SSH client should be available in Terminal (Mac Terminal, Git Bash, or WSL) by default. 
...
- Open Terminal (Mac Terminal, Git Bash, or WSL) at the `/Docker` folder of course material. Type
```{bash}
#| eval: false
...
Much of homework and in class examples are demonstrated on the MIMIC-IV v1.0 data set. Download the data to your computer from  [here](https://ucla.box.com/s/id1qge4ehykjtbeio927ofxi4zhy13z9) (7.76 GB, need UCLA log in), and make it available at `~/mimic`. For example, you can create a symbolic link by
```{bash}
#| eval: false
ln -s /PATH/TO/YOUR/MIMIC_FOLDER ~/mimic
```
Your homework solution should always reading data from `~/mimic`. This is critical for Tomoki and me to reproduce your homework.
```{bash}
ls ~/mimic
```